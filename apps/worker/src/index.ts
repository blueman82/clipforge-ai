...
const compositionWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    try {
      ...
      const previewVideo = await videoCompositionService.composeVideo(script, assets, audioData, {
        resolution: '720x1280',
        quality: 'low',
      })
...
console.log(
  'Listening for jobs on queues: script-generation, tts-generation, asset-selection, video-composition, video-export'
)
